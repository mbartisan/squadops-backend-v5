...
function makeTable(tableName, schema) {
  ...
  async function findAll(searchData, props = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (searchData == null || Object.keys(searchData).length === 0) {
        const sql = `SELECT ${sanitizeSelectProps(props)} FROM ${pool.escapeId(tableName)}`;
        ...
        const sql = `SELECT ${sanitizeSelectProps(props)} FROM ${pool.escapeId(tableName)} WHERE ${sanitizeWhereObject(searchData)}`;
        pool.query(sql, (error, results, fields) => {
          if (error) {
  ...
  async function findOne(searchData, props = null) {
    ...
    return new Promise((resolve, reject) => {
      if (props == null || props.length === 0) props = '*';
      const sql = `SELECT ${sanitizeSelectProps(props)} FROM ${pool.escapeId(tableName)} WHERE ${sanitizeWhereObject(searchData)} LIMIT 1`;
      pool.query(sql, (error, results, fields) => {
        if (error) {
          console.error(`SQL ERROR: ${sql}`, searchData);
          return reject(error);
  ...
  async function updateWhere(updateData, whereData) {
    ...
    return new Promise((resolve, reject) => {
      pool.getConnection((error, connection) => {
        ...
        const sql = `UPDATE ${pool.escapeId(tableName)} SET ? WHERE ${sanitizeWhereObject(whereData)}`;
  ...
  async function insert(insertData) {
    ...
    return new Promise((resolve, reject) => {
      pool.getConnection((error, connection) => {
        if (error) return reject(error);
        const sql = `INSERT INTO ${pool.escapeId(tableName)} SET ?`;
  ...
  async function deleteWhere(whereData) {
    ...
    return new Promise((resolve, reject) => {
      const sql = `DELETE FROM ${pool.escapeId(tableName)} WHERE ${sanitizeWhereObject(whereData)}`;
      pool.query(sql, (error, results, fields) => {
        if (error) {
  ...
  async function search(searchProperty, searchString, returnProps, { searchLeft = true, searchRight = true }) {
    ...
    return new Promise((resolve, reject) => {
      const sql = `SELECT ${sanitizeSelectProps(returnProps)} FROM ${pool.escapeId(tableName)} WHERE ${pool.escapeId(searchProperty)} LIKE '${(searchLeft) ? '%' : ''}${pool.escape(searchString)}${(searchRight) ? '%' : ''}'`;
